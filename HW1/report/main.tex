...
\begin{itemize}
  ...
  \item \textbf{Boundary:} Balloon condition on a large surrounding region.
...
\textit{[Insert Maxwell flux-density vector plots exported at
$\theta = 0^{\circ}$, $45^{\circ}$, and $90^{\circ}$.]}
...
\textit{[Insert Maxwell flux-density vector plots at $0^{\circ}$,
$45^{\circ}$, and $90^{\circ}$ for the nonlinear simulation.]}

\subsection{Inductance and Stored Energy}

\textit{[Insert results table for the nonlinear case.  Note that
$W = \tfrac{1}{2}LI^{2}$ is only an approximation here; the true
stored energy must be computed from the co-energy integral.]}

\subsection{Torque vs.\ Angle}

\textit{[Insert torque vs.\ angle plot for the nonlinear simulation,
compared with the linear FEA and analytical results.]}

\subsection{Discussion}

\textit{[Compare nonlinear vs.\ linear FEA.  Expected effects:
saturation at the pole tips ($B > 1.5\unit{T}$) will reduce the
effective permeability locally, lowering $L_{\max}$ compared to the
linear case.  Discuss how fringing and saturation interact and whether
the torque peak changes significantly.  Reference the $B$--$H$ curve in
Appendix~\ref{app:bh}..]}
...
engineering purposes.  Nonlinear FEA (Q3) is expected to reduce
$L_{\max}$ further due to core saturation.  Switching the DC excitation
on during the productive stroke raises the average torque to
approximately $33.8\unit{mN\cdot m}$.
...
\textit{[Insert the $B$--$H$ curve table or plot for steel\_1010 as
exported from the ANSYS Maxwell material library.]}
